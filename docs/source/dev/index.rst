...
The Dungeon Nexus project implements a regression
...
not broken existing functionality. The Dungeon Nexus
...
.. code-block:: console
    ...
    (dungeon_nexus) $ make test-verbose
...
``dungeon_nexus`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``dungeon_nexus`` package can be found.

.. code-block:: console

    (dungeon_nexus) $ cd tests
    (dungeon_nexus) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (dungeon_nexus) $ python -m test_version
...
.. code-block:: console
    ...
    (dungeon_nexus) $ make sort-imports
...
.. code-block:: console
    ...
    (dungeon_nexus) $ make check-types
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/dungeon_nexus/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (dungeon_nexus) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (dungeon_nexus) $ make dist-upload

  The package should now be available at https://pypi.org/project/dungeon_nexus/
